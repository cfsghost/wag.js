...
Wag._resolveLookupPath = function(request, parent, caller) {
	...
	if (request.substring(0, 2) == './' || request.substring(0, 3) == '../') {
		var base = path.dirname(caller.filePath);
...
Wag.prototype.mixin = function() {
	...
	var caller = arguments[1] || callerId.getData();
	...
	// Finding path
	var enginePaths = Wag._resolveLookupPath(engineName, self, caller);

	// This engine was loadded already
	if (enginePaths == null)
		return self;
	...
	// Load engine
	for (var index in enginePaths) {
		var engine = self.loadEngine(engineName, enginePaths[index]);
	}
